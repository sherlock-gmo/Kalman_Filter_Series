...
# Predictor en forma de estados
	...
	def f2(self,P,Mh1,Mh2):
		S,M,DM,sDM,a,c,b,d,alpha1,alpha2 = P
		DMh2 = (alpha1-a)*DM+alpha2*M-c*sDM+b*S+d-alpha2*Mh2-alpha1*Mh1
		return DMh2
#----------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------
	def predictor(self,Par_q0,V,ALPHA,GAMMA,DM_hat0,M_hat0,h):
		...
		P = [S,M,DM,sDM,a0,c0,b0,d0,alpha1,alpha2]
		DM_hat, M_hat = self.runge_kutta04_2ord(P,DM_hat0,M_hat0,h)
		e1 = M_hat-M
		e2 = DM_hat-DM
		# Ley de adaptacion
		gamma1,gamma2,gamma3,gamma4 = GAMMA
		a = self.runge_kutta04_1ord(a0,1.0,gamma1,e2,DM,h)
		c = self.runge_kutta04_1ord(c0,1.0,gamma2,e2,sDM,h)
		b = self.runge_kutta04_1ord(b0,-1.0,gamma3,e2,S,h)
		d = self.runge_kutta04_1ord(d0,-1.0,gamma4,e2,1.0,h)
		return a, c, b, d, DM_hat, M_hat
