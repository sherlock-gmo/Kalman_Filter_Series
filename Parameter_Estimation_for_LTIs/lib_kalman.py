...
class linear_kalman_filter():
	...
	def Scovarianze_act(self,P,H,Kn,R,N):
		# N = Numero de estados
		I = np.eye(N)
		p1 = I-np.matmul(Kn,H)
		p2 = np.matmul(p1,P)
	...
	def Skalman_offline(self,X0,P0,F,B,u,H,R,Q,Z):
		Xk = []
		i = 0
		# Ajuste de tamanos
		N,_ = X0.shape 
	...
	def parameters_ext(self,h):
		h_ = h
		return h_
	# Extrapolacion de la covarianza
	def Pcovariance_ext(self,P,lambda0):
		Rnn = ((1.0/lambda0)-1.0)*Pnn
		Pn1n = Pnn+Rnn
		return Pn1n
	# Actualizacion de la ganancia de Kalman
	def PkalmanG_act(self,P,H,re):
		Ht = np.transpose(H)
		k1 = np.matmul(P,Ht)
		k2 = np.matmul(H,P)
		k3 = np.matmul(k2,Ht)
	...
	def parameters_act(self,Kn,H,h,X,y):
		x1 = np.matmul(H,X)
		x2 = np.matmul(Kn,y-x1) 
		hnn = h+x2
		return hnn
	# Actualizacion de la covarianza
	def Pcovarianze_act(self,P,H,Kn,R,N):
		# N = Numero de estados
		I = np.eye(N)
		p1 = I-np.matmul(Kn,H)
		Pnn = np.matmul(p1,P)
